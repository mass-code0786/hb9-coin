  ...
  pages['HB9 Exchange'] = function(){
    ...
    const historyRows = (data.conversions || [])
      ...
      .map(x => {
        ...
        return `<tr><td>${String(x.createdAt || '').slice(0, 10)}</td><td>${esc(fromAsset)}</td><td>${assetLogo(toAsset)} ${esc(toAsset)}</td><td>${money(fromAmount)}</td><td>${points(toAmount)} ${esc(toAsset)}</td><td>${money(x.price ?? x.buyPrice ?? x.rate ?? 0)}</td><td>${esc(x.status || 'completed')}</td></tr>`;
      }).join('');
