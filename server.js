...
const server=http.createServer(async(req,res)=>{
  try {
    if (req.url.startsWith('/api/')) {
      ...
      if (p==='/api/auth/register'&&method==='POST') { const {name,email,password,sponsorEmail,walletAddress}=await body(req); if(!name||!email||!password||password.length<8)return send(res,400,{error:'Name, email and 8+ character password are required'}); if(walletAddress&&typeof walletAddress==='string'&&!/^0x[a-fA-F0-9]{40}$/.test(walletAddress))return send(res,400,{error:'Enter a valid 42-character BEP20 wallet address starting with 0x'}); if(db.users.some(x=>x.email.toLowerCase()===email.toLowerCase()))return send(res,409,{error:'Email already registered'}); const h=hash(password), sponsorCode=String(sponsorEmail||'').trim().toLowerCase(), sponsor=db.users.find(x=>String(x.email||'').toLowerCase()===sponsorCode); const u={id:id('usr'),name,email:email.toLowerCase(),role:'user',status:'active',passwordHash:h.hash,salt:h.salt,walletAddress:walletAddress||null,sponsorId:sponsor?.id||null,createdAt:new Date().toISOString()}; db.users.push(u);writeDB(db);return send(res,201,{message:'Registration complete. Please log in.'}); }
