...
async function convertUsdtToAsset(db,user,{amount,toAsset='HB9',clientRequestId=null}={}){
  if(!db.settings.exchangeEnabled)throw Error('Exchange is disabled');
  const value=roundCurrency(Number(amount)), normalized=String(toAsset||'HB9').toUpperCase();
  ...
  if(clientRequestId&&db.exchange_orders?.some(x=>x.userId===user.id&&x.clientRequestId===clientRequestId))return {duplicate:true,order:db.exchange_orders.find(x=>x.userId===user.id&&x.clientRequestId===clientRequestId),balance:walletBalances(db,user.id)};
  ...
  if(normalized==='HB9'&&reserveWallet(db,'HB9','exchange').balance<toAmount)throw Error('HB9 reserve is insufficient');
  const orderId=id('xord'),createdAt=new Date().toISOString();
  if(normalized==='HB9')reserveMove(db,{asset:'HB9',walletType:'exchange',direction:'debit',amount:toAmount,reason:'HB9 buy',userId:user.id,refId:orderId});
...
async function createStake(db,user,{amount,stakeAsset='HB9',clientRequestId=null}={}){
  ...
  if(user.sponsorId){
    const referralPercent=setting(db,'referralPercent'),referralUsdAmount=roundCurrency(usdAmount*referralPercent/100),referralHb9Amount=payoutPrice>0?roundCurrency(referralUsdAmount/payoutPrice):0,refId=id('ref');
    ...
    db.referralLedger=db.referralLedger||[];db.income_emissions=db.income_emissions||[];
    db.referralLedger.push({id:refId,type:'REFERRAL_INCOME',asset:'HB9',sponsorId:user.sponsorId,referredUserId:user.id,stakeId:stake.id,stakeAsset:normalized,stakeAmount:usdAmount,stakeCoinAmount:hb9EquivalentAmount,referralPercent,referralAmount:creditedHb9,referralHb9Amount:creditedHb9,queuedHb9Amount:status==='queued'?referralHb9Amount:0,referralUsdAmount,hb9PriceAtCredit:payoutPrice,hb9PriceAtPayout:payoutPrice,status,note,date:today(),createdAt,immutable:true});
...
const server=http.createServer(async(req,res)=>{
  try {
    if (req.url.startsWith('/api/')) {
      ...
      if(p==='/api/convert'&&method==='POST'){try{const input=await body(req),result=await convertUsdtToAsset(db,u,input);writeDB(db);return send(res,result.duplicate?200:201,{message:result.duplicate?'Conversion already completed':`USDT converted to ${result.order.toAsset}`,order:result.order,balance:result.balance,hb9Amount:result.order.hb9Amount,bnbAmount:result.order.bnbAmount,buyPrice:result.order.buyPrice});}catch(error){return send(res,/disabled/.test(error.message)?403:400,{error:error.message});}}
