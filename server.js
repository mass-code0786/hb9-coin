...
const NOWPAYMENTS_DEPOSIT_EXPIRY_MS = 20*60*1000;
...
const WITHDRAWAL_POLL_MS = Math.max(5000, Number(process.env.WITHDRAWAL_POLL_MS || 15000));
...
let withdrawalRunning = false;
let withdrawalNonceQueue = Promise.resolve();
const DEPOSIT_RUNTIME_MODE = { provider:'NOWPayments', hdWatcherEnabled:false };
...
const SALARY_SCHEDULER_DATES = [1,16];
...
const SALARY_SCHEDULER_UTC = parseUtcTime(process.env.SALARY_SCHEDULER_UTC, 0, 5);
...
function syncHb9FallbackPrice(db){
  if(process.env.MARKET_TEST_MODE==='true')return TEST_HB9_PRICE;
  const envFallback=envHb9FallbackPrice();
...
function emptyDB(){
  ...
  return {
    ...
    hb9_market_settings:{fallbackPrice:envHb9FallbackPrice(),priceOffset:DEFAULT_PRICE_OFFSET,spreadPercent:5,manualOverrideEnabled:false,updatedBy:'system',updatedAt:now},
    hb9_price_history:[],
...
function seedDevOnlyDemo() {
  ...
  writeDB({
    ...
    conversions:[{id:'cnv_demo',userId:'usr_alice',direction:'buy',usdtAmount:100,hb9Amount:roundCurrency(100/TEST_HB9_PRICE),rate:TEST_HB9_PRICE,buyPrice:TEST_HB9_PRICE,sellPrice:roundCurrency(TEST_HB9_PRICE-DEFAULT_PRICE_OFFSET),createdAt:now}],
    stakes:[{id:'stk_demo',userId:'usr_alice',amount:100,usdValueAtStake:100,coinAmount:roundCurrency(100/TEST_HB9_PRICE),hb9Amount:roundCurrency(100/TEST_HB9_PRICE),hb9PriceAtStake:TEST_HB9_PRICE,status:'active',startDate:datePlus(today(),-8),dailyRate:0.02,createdAt:now}],
    ...
    hb9_market_settings:{fallbackPrice:envHb9FallbackPrice(),priceOffset:DEFAULT_PRICE_OFFSET,spreadPercent:5,manualOverrideEnabled:false,updatedBy:'usr_admin',updatedAt:now},
    hb9_price_history:[],
    settings:{...baseSettings(),treasuryWalletBSC:'0x9999999999999999999999999999999999999999'}
  });
}
function setting(db, key) { return db.settings[key]; }
...
function marketSettings(db){
  ...
  const envFallback=envHb9FallbackPrice();
  const storedFallback=Number(db.hb9_market_settings.fallbackPrice ?? db.settings?.fallbackPrice);
  const manual=Boolean(db.hb9_market_settings.manualOverrideEnabled ?? db.settings?.manualOverrideEnabled);
  db.hb9_market_settings.fallbackPrice=envFallback ?? (manual&&Number.isFinite(storedFallback)&&storedFallback>0?storedFallback:null);
  ...
  db.hb9_market_settings.manualOverrideEnabled=manual;
...
function setMarketSettings(db,{fallbackPrice,buyPrice,priceOffset,spreadPercent,manualOverrideEnabled,buyFeePercent,sellFeePercent},updatedBy){
  ...
  Object.assign(db.settings,{fallbackPrice:roundCurrency(fallback),priceOffset:roundCurrency(offset),spreadPercent:spread,manualOverrideEnabled:manual,buyFeePercent:buyFee,sellFeePercent:sellFee,hb9Price:null,priceMode:manual?'manual_override':'icp_proxy'});
  db.hb9_price_history=db.hb9_price_history||[];
...
async function exchangeMarket(db,interval='1d',limit=120){
  ...
  if(process.env.MARKET_TEST_MODE==='true'){
    const icpPrice=Number(settings.fallbackPrice)||TEST_HB9_PRICE, candles=Array.from({length:limit},(_,index)=>{const open=icpPrice+(index%5)*.001,close=open+(index%2?.0015:-.001);return {time:Date.now()-(limit-index)*86400000,open,high:open+.003,low:open-.003,close,volume:1000+index}});
  ...
  if(manual){
    if(!Number.isFinite(Number(settings.fallbackPrice))||Number(settings.fallbackPrice)<=0)throw Error('Manual HB9 price is unavailable');
...
function activeStakeUsdAsOf(db,userId,asOf=null){return activeStakes(db,userId).filter(stake=>beforeCutoff(stake,asOf)).reduce((n,s)=>n+(Number(s.usdValueAtStake)||Number(s.amount)||0),0);}
function salaryDirectCount(db,userId,minStakeUsd,asOf=null){return (db.users||[]).filter(u=>u.sponsorId===userId&&activeStakeUsdAsOf(db,u.id,asOf)>=minStakeUsd).length;}
function downlineUsers(db,userId,maxLevel=20){
  const rootId=String(userId), result=[], queue=[{userId:rootId,level:0}], visited=new Set([rootId]);
  while(queue.length){
    const current=queue.shift();
    if(current.level>=maxLevel)continue;
    (db.users||[]).filter(u=>u.sponsorId===current.userId&&u.role==='user').forEach(child=>{
      if(visited.has(child.id))return;
      visited.add(child.id);
      const level=current.level+1;
      result.push({user:child,level});
      queue.push({userId:child.id,level});
    });
  }
  return result;
}
function stakeInvestmentSummary(db,userId){
  const stakes=(db.stakes||[]).filter(stake=>stake.userId===userId), active=stakes.filter(stake=>stake.status==='active');
...
function normalizedIncomeLedgerType(record){
  return String(record?.type||record?.incomeType||record?.income_type||record?.kind||'').trim().toUpperCase().replace(/[\s-]+/g,'_');
}
function isB1IncomeLedgerEntry(record){
  const type=normalizedIncomeLedgerType(record);
  return type==='B1_INCOME'||type==='B1'||type==='B1INCOME';
}
function isCreditedIncomeLedgerEntry(record){
  const status=String(record?.status||'credited').trim().toLowerCase();
  return ['credited','credit','paid','partial','repaired'].includes(status);
}
function b1IncomeLedgerAmount(record){
  return roundCurrency(Number(record?.paidB1Hb9)||Number(record?.hb9Amount)||Number(record?.creditedB1Hb9)||Number(record?.amount)||Number(record?.creditAmount)||0);
}
function incomeLedgerDate(record){
  return String(record?.date||record?.incomeDate||record?.createdAt||'').slice(0,10);
}
function b1DisplayStatus(record){
  const status=String(record?.status||'').toLowerCase();
  if(status==='flushed'||Number(record?.paidB1Usd||record?.creditedB1Usd||0)<=0)return 'Flushed';
  if(status==='partial'||Number(record?.flushedB1Usd||record?.unqualifiedB1FlushUsd||0)>0)return 'Partial';
  return 'Paid';
}
function salaryPayoutStatus(record){
  return String(record?.status||'credited').trim().toLowerCase();
}
function isCreditedSalaryPayout(record){
...
function salaryDisplayStatus(record){
  ...
  if(isCreditedSalaryPayout(record))return 'Credited';
...
function userIncomeSummary(db,userId){
  ...
  const b1=(db.incomeLedger||[]).filter(x=>x.userId===userId&&isB1IncomeLedgerEntry(x)&&isCreditedIncomeLedgerEntry(x));
  const salary=creditedSalaryPayouts(db,userId);
  const amount=x=>isB1IncomeLedgerEntry(x)?b1IncomeLedgerAmount(x):Number(x.referralHb9Amount)||Number(x.referralAmount)||Number(x.hb9Amount)||Number(x.amount)||0;
...
function incomeHistory(db,userId){
  const userName=id=>userById(db,id)?.name||id||'System';
  const entries=[];
...
function teamLevelsReport(db,userId,maxLevel=20){
  ...
  const levels=Array.from({length:maxLevel},(_,index)=>{
    const level=index+1, members=grouped.get(level)||[];
    return {level,memberCount:members.length,totalInvestmentUsd:roundCurrency(members.reduce((sum,member)=>sum+(Number(member.totalInvestmentUsd)||0),0)),totalStakeHb9Equivalent:roundCurrency(members.reduce((sum,member)=>sum+(Number(member.hb9EquivalentAmount)||0),0)),members};
  });
  const summary={totalLevels:maxLevel,totalMembers:levels.reduce((sum,level)=>sum+level.memberCount,0),activeMembers:levels.reduce((sum,level)=>sum+level.members.filter(member=>member.active).length,0),totalInvestmentUsd:roundCurrency(levels.reduce((sum,level)=>sum+level.totalInvestmentUsd,0)),totalHb9Equivalent:roundCurrency(levels.reduce((sum,level)=>sum+level.totalStakeHb9Equivalent,0))};
  return {levels,summary};
}
function salaryTeamBusinessUsd(db,userId,asOf=null){return roundCurrency(downlineUsers(db,userId,20).reduce((sum,item)=>sum+activeStakeUsdAsOf(db,item.user.id,asOf),0));}
function totalSalaryPaidUsd(db,userId){ensureSalaryTables(db);return roundCurrency(creditedSalaryPayouts(db,userId).reduce((n,x)=>n+(Number(x.usdAmount)||0),0));}
function salaryProgressForRank(db,userId,rank,{asOf=null}={}){
  const personalInvestment=roundCurrency(activeStakeUsdAsOf(db,userId,asOf)), directCount=salaryDirectCount(db,userId,rank.directMinStakeUsd,asOf), teamBusiness=salaryTeamBusinessUsd(db,userId,asOf);
...
function salaryReport(db,userId,{asOf=null}={}){
  ensureSalaryTables(db);
  const ranks=db.salary_ranks.map(rank=>salaryProgressForRank(db,userId,rank,{asOf}));
  const current=[...ranks].reverse().find(x=>x.qualified)||null, next=ranks.find(x=>!x.qualified)||null;
  const personalInvestment=roundCurrency(activeStakeUsdAsOf(db,userId,asOf)), maxSalaryCapUsd=roundCurrency(personalInvestment*3), totalPaidUsd=totalSalaryPaidUsd(db,userId), remainingCapUsd=roundCurrency(Math.max(0,maxSalaryCapUsd-totalPaidUsd));
  const now=new Date().toISOString();
...
async function processSalaryPayouts(db,date=today()){
  ...
  const summary={date:periodDate,salaryPeriodDate:periodDate,hb9Price:null,processedUsers:0,creditedUsers:0,queuedUsers:0,cappedUsers:0,totalSalaryUsd:0,totalSalaryHb9:0,skippedUsers:0,notEligibleUsers:0,duplicateUsers:0,runDateAllowed:isSalaryRunDate(periodDate)};
  audit(db,'SALARY_DAILY_CHECK',{salaryPeriodDate:periodDate,allowed:summary.runDateAllowed,dates:SALARY_SCHEDULER_DATES,timeUtc:SALARY_SCHEDULER_UTC.label});
...
function distributeStakeIncome(db,user,stake,payoutPrice,isFirstStake){
  ...
  const summary={referralCreated:0,referralSkippedDuplicate:0,directBusinessCreated:0,directBusinessSkippedDuplicate:0};
  ...
  if(user.sponsorId){
    ...
    }else{summary.referralSkippedDuplicate++;audit(db,'REFERRAL_INCOME_SKIPPED_DUPLICATE',{sponsorId:user.sponsorId,referredUserId:user.id,stakeId:stake.id});}
    if(!db.directBusiness.some(x=>x.userId===user.sponsorId&&x.sourceUserId===user.id&&x.stakeId===stake.id)){
      ...
      summary.directBusinessCreated++;
    }else summary.directBusinessSkippedDuplicate++;
  }
...
function walletBalances(db,userId) {
  ...
  const withdrawals=(db.withdrawals||[]).filter(x=>x.userId===userId&&!['rejected','failed'].includes(String(x.status||''))).reduce((n,x)=>n+x.amount,0);
  const b1Hb9=(db.incomeLedger||[]).filter(x=>x.userId===userId&&isB1IncomeLedgerEntry(x)&&isCreditedIncomeLedgerEntry(x)).reduce((n,x)=>n+b1IncomeLedgerAmount(x),0);
  ...
  const salaryHb9=creditedSalaryPayouts(db,userId).reduce((n,x)=>n+salaryPayoutAmount(x),0);
...
function audit(db,type,details){db.auditLogs=db.auditLogs||[];const record={id:id('aud'),type,details,createdAt:new Date().toISOString()};db.auditLogs.push(record);if(/^(GLOBAL_TEAM|ROI|REFERRAL_INCOME|B1|HB9_PRICE|SALARY)_/.test(type))console.log(type,{...details,createdAt:record.createdAt});}
...
async function createNowPaymentsDeposit(db,userId,amount){
  ...
  const depositId=id('dep'),now=new Date().toISOString(),expiresAt=new Date(Date.now()+NOWPAYMENTS_DEPOSIT_EXPIRY_MS).toISOString();
...
function nowPaymentFinalTime(payload,nowIso){
  const candidates=[payload.payment_finished_at,payload.finished_at,payload.payment_updated_at,payload.updated_at,payload.created_at,payload.payment_created_at];
  const value=candidates.find(x=>x&&!Number.isNaN(Date.parse(x)));
  return value?new Date(value).toISOString():nowIso;
}
function markExpiredNowPaymentsDeposits(db,nowIso=new Date().toISOString()){
  let changed=false;
  const nowMs=Date.parse(nowIso);
  (db.deposits||[]).forEach(deposit=>{
    const expiresMs=Date.parse(deposit.expiresAt||'');
    if(deposit.provider==='NOWPayments'&&!deposit.credited&&['pending','confirming'].includes(String(deposit.status||'').toLowerCase())&&Number.isFinite(expiresMs)&&Number.isFinite(nowMs)&&nowMs>expiresMs){
      Object.assign(deposit,{status:'expired',paymentStatus:deposit.paymentStatus==='finished'||deposit.paymentStatus==='confirmed'?deposit.paymentStatus:'expired',expiredAt:deposit.expiredAt||nowIso});
      changed=true;
    }
  });
  return changed;
...
function creditNowPaymentsDeposit(db,payload){
  ...
  if(['failed','expired','refunded'].includes(status)){
    Object.assign(deposit,{status:status==='expired'?'expired':'failed',failedAt:status==='expired'?deposit.failedAt:deposit.failedAt||now,expiredAt:status==='expired'?(deposit.expiredAt||now):deposit.expiredAt});
  ...
  const finalTime=nowPaymentFinalTime(payload,now),expiresMs=Date.parse(deposit.expiresAt||''),finalMs=Date.parse(finalTime);
  if(Number.isFinite(expiresMs)&&Number.isFinite(finalMs)&&finalMs>expiresMs&&!deposit.credited){
    Object.assign(deposit,{status:'expired',paymentStatus:status,expiredAt:deposit.expiredAt||now,latePaymentAt:finalTime,latePaymentStatus:status});
    audit(db,'NOWPAYMENTS_DEPOSIT_LATE_EXPIRED',{depositId:deposit.id,userId:deposit.userId,paymentId,invoiceId,orderId,expiresAt:deposit.expiresAt,finalTime,status});
    return {deposit,credited:false,reason:'late_after_expiry'};
  }
...
async function assetBuyPrice(db,asset){
  ...
  if(normalized==='HB9'){const priceInfo=await hb9PriceSource(db);return {asset:'HB9',price:priceInfo.buyPrice,market:priceInfo.market};}
...
async function createStake(db,user,{amount,stakeAsset='HB9',clientRequestId=null}={}){
  ...
  const hb9PriceInfo=await hb9PriceSource(db), hb9Price=hb9PriceInfo.buyPrice, payoutPrice=hb9PriceInfo.price;
...
async function pollDepositWatcher(){
  if(!DEPOSIT_RUNTIME_MODE.hdWatcherEnabled)return {disabled:true,provider:DEPOSIT_RUNTIME_MODE.provider};
...
function withdrawalProvider(){return new JsonRpcProvider(process.env.BSC_RPC_URL);}
function withdrawalSigner(provider){const signer=new Wallet(process.env.WITHDRAWAL_PRIVATE_KEY,provider);if(process.env.WITHDRAWAL_FROM_ADDRESS&&getAddress(process.env.WITHDRAWAL_FROM_ADDRESS)!==getAddress(signer.address))throw Error('WITHDRAWAL_FROM_ADDRESS does not match WITHDRAWAL_PRIVATE_KEY');return signer;}
function withdrawalToken(signer){return new Contract(getAddress(process.env.USDT_BEP20_CONTRACT),['function balanceOf(address) view returns (uint256)','function transfer(address,uint256) returns (bool)'],signer);}
function withWithdrawalNonceLock(task){
  const run=withdrawalNonceQueue.then(task,task);
  withdrawalNonceQueue=run.catch(()=>{});
  return run;
}
...
async function updateWithdrawalConfirmations(db,withdrawal,provider){
  ...
  const receipt=await provider.getTransactionReceipt(withdrawal.txHash);if(!receipt)return false;
  if(Number(receipt.status)!==1){failWithdrawal(db,withdrawal,'USDT withdrawal transaction reverted');return true;}
  const latest=await provider.getBlockNumber(),required=Number(withdrawal.requiredConfirmations||withdrawalConfirmationsRequired()),confirmations=Math.max(0,Number(latest)-Number(receipt.blockNumber)+1);
  withdrawal.confirmations=confirmations;withdrawal.updatedAt=new Date().toISOString();
...
async function pollWithdrawalWorker(){
  ...
  try{
    ...
    writeDB(db);
  }catch(error){console.error('Withdrawal worker error:',error.message);}finally{withdrawalRunning=false;}
}
function startWithdrawalWorker(){if(withdrawalTimer||process.env.WITHDRAWAL_WORKER_DISABLED==='true'||!withdrawalHotWalletStatus().configured)return false;withdrawalTimer=setInterval(pollWithdrawalWorker,WITHDRAWAL_POLL_MS);return true;}
...
function incomeContext(db,userId,date,hb9PriceOverride=null,{asOf=null}={}) {
  ...
  const creditedB1Usd=paidB1Usd, paidB1Hb9=hb9Price>0?roundCurrency(paidB1Usd/hb9Price):0;
  const creditedB1Hb9=paidB1Hb9;
...
function globalForDate(db,userId,date,hb9PriceOverride=null,{roi=true,logDuplicates=false,asOf=null}={}) {
  ...
  for(const item of stakeContexts){
    let incomeStatus=item.status, paidB1Hb9=item.paidB1Hb9, paidB1Usd=item.paidB1Usd, incomeQueuedReason=null;
    if((db.incomeLedger||[]).some(record=>record.incomeKey===item.incomeKey||(record.userId===userId&&record.stakeId===item.stake.id&&record.date===date&&isB1IncomeLedgerEntry(record))))continue;
    if(item.paidB1Hb9>0){
      try{
        reserveMove(db,{asset:'HB9',walletType:'income',direction:'debit',amount:item.paidB1Hb9,reason:'B1 income emission',userId,refId:item.incomeKey});
  ...
  const flushRecord={incomeType:'B1 / Global Team',eligibleIncome:context.grossB1Usd,paidIncome:paid,flushedIncome:context.flushUsd,burnStatus:'Burned Forever',withdrawable:false,recoverable:false,...context,queuedB1Usd:queuedUsd,queuedB1Hb9:queuedHb9,createdAt};
  ...
  audit(db,'B1_INCOME_CREATED',{userId,date,rows:createdLedgerRows.length,paidB1Hb9:roundCurrency(createdLedgerRows.reduce((sum,row)=>sum+b1IncomeLedgerAmount(row),0)),paidB1Usd:paid,flushedB1Usd:context.flushedB1Usd,activeStakeUsd:context.activeStakeUsd,directBusinessUsd:context.directBusinessUsd,qualifiedStakeUsd:context.qualifiedStakeUsd,dailyB1Percent:context.dailyB1Percent});
...
function accrueGlobalPoints(db,{userId=null,fromDate=null,toDate=today(),hb9PriceOverride=null,roi=true,logDuplicates=false,now=new Date()}={}){
  ...
  return {processedUsers,createdDays,skippedDays,globalGenerated:db.globalTeamRecords.length-before.global,flushGenerated:db.flushRecords.length-before.flush,b1Credited:db.incomeLedger.slice(before.ledger).filter(isCreditedIncomeLedgerEntry).reduce((n,x)=>n+b1IncomeLedgerAmount(x),0)};
}
...
async function marketPayoutPrice(db){return (await hb9PriceSource(db,{interval:'1d',limit:1})).price;}
...
let globalTeamTimer=null, roiTimer=null, salaryTimer=null, schedulerRunning=false;
...
function nextSalaryDueTime(now=new Date()){
  const current=new Date(now);
  for(let monthOffset=0;monthOffset<14;monthOffset++){
    const year=current.getUTCFullYear(), month=current.getUTCMonth()+monthOffset;
    for(const day of SALARY_SCHEDULER_DATES){
      const candidate=new Date(Date.UTC(year,month,day,SALARY_SCHEDULER_UTC.hour,SALARY_SCHEDULER_UTC.minute,0,0));
      if(candidate>now)return candidate;
    }
  }
  return new Date(now.getTime()+24*60*60*1000);
}
function scheduleSalaryTimer(){
  const delay=Math.max(1000,nextSalaryDueTime(new Date())-new Date());
  return setTimeout(async()=>{try{await executeScheduledRun('salary',{now:new Date()});}catch(error){console.error('salary scheduler error:',error.message);}finally{salaryTimer=scheduleSalaryTimer();}},delay);
}
async function startDailySchedulers(){
  ...
  try{
    ...
    const salaryNow=new Date(), salaryDate=utcDate(salaryNow), salaryDue=utcDateTime(salaryDate,SALARY_SCHEDULER_UTC.hour,SALARY_SCHEDULER_UTC.minute);
    if(isSalaryRunDate(salaryDate)&&salaryNow>=salaryDue)await executeScheduledRun('salary',{now:salaryNow});
    else{const db=readDB();audit(db,'SALARY_DAILY_CHECK',{salaryPeriodDate:salaryDate,allowed:false,dates:SALARY_SCHEDULER_DATES,timeUtc:SALARY_SCHEDULER_UTC.label,reason:isSalaryRunDate(salaryDate)?'before_scheduled_time':'not_salary_date'});writeDB(db);}
  ...
  salaryTimer=scheduleSalaryTimer();
}
async function repairReferralB1Income(db,{userSearch=null,fromDate=null,toDate=null,runB1=true}={}){
  ensureSupply(db);
  db.referralLedger=db.referralLedger||[];db.directBusiness=db.directBusiness||[];db.level_income_ledger=db.level_income_ledger||[];db.incomeLedger=db.incomeLedger||[];db.auditLogs=db.auditLogs||[];
  const currentPayoutPrice=await marketPayoutPrice(db);
  const query=String(userSearch||'').trim().toLowerCase(), users=db.users||[], stakes=(db.stakes||[]).slice().sort((a,b)=>String(a.createdAt||a.startDate||'').localeCompare(String(b.createdAt||b.startDate||'')));
  const before={referralLedger:db.referralLedger.length,directBusiness:db.directBusiness.length,levelIncome:db.level_income_ledger.length,incomeLedger:db.incomeLedger.length,flushRecords:(db.flushRecords||[]).length};
  const inspectedUsers=[], repairedStakes=[];
  for(const stake of stakes){
    const user=userById(db,stake.userId);
    if(!user||!user.sponsorId)continue;
    const sponsor=userById(db,user.sponsorId);
    if(query&&!`${user.id} ${user.name||''} ${user.email||''} ${sponsor?.id||''} ${sponsor?.name||''} ${sponsor?.email||''}`.toLowerCase().includes(query))continue;
    const userStakes=stakes.filter(item=>item.userId===user.id), firstStake=userStakes[0]?.id===stake.id, payoutPrice=Number(stake.hb9PriceAtPayout)||currentPayoutPrice;
    inspectedUsers.push({id:user.id,name:user.name,email:user.email,sponsorId:user.sponsorId,sponsorName:sponsor?.name||null,sponsorEmail:sponsor?.email||null,stakeId:stake.id,stakeAsset:stake.stakeAsset||'HB9',stakeAmount:Number(stake.stakeAmount)||Number(stake.coinAmount)||0,stakeUsdValue:Number(stake.usdValueAtStake)||Number(stake.amount)||0,status:stake.status,startDate:stake.startDate,createdAt:stake.createdAt,hasReferral:db.referralLedger.some(x=>x.stakeId===stake.id&&x.type==='REFERRAL_INCOME'),hasDirectBusiness:db.directBusiness.some(x=>x.userId===user.sponsorId&&x.sourceUserId===user.id&&x.stakeId===stake.id)});
  ...
  if(runB1&&dueStart<=dueEnd){
    ...
    roiSummary=await runRoiDaily(db,{fromDate:dueStart,toDate:dueEnd,backfill:true,logStartComplete:true});
  }
  const after={referralLedger:db.referralLedger.length,directBusiness:db.directBusiness.length,levelIncome:db.level_income_ledger.length,incomeLedger:db.incomeLedger.length,flushRecords:(db.flushRecords||[]).length};
  return {query:query||null,inspectedUsers,repairedStakes,b1Backfill:{runB1,fromDate:dueStart,toDate:dueEnd,globalSummary,roiSummary},before,after,created:{referralLedger:after.referralLedger-before.referralLedger,directBusiness:after.directBusiness-before.directBusiness,levelIncome:after.levelIncome-before.levelIncome,incomeLedger:after.incomeLedger-before.incomeLedger,flushRecords:after.flushRecords-before.flushRecords}};
}
...
function dashboard(db,u) {
  ...
  const salary=salaryReport(db,u.id), salaryPayouts=creditedSalaryPayouts(db,u.id);
...
const server=http.createServer(async(req,res)=>{
  try {
    if (req.url.startsWith('/api/')) {
      ...
      if(p==='/api/market/hb9-usdt'&&method==='GET'){const priceInfo=await hb9PriceSource(db);return send(res,200,{symbol:'HB9/USDT',...priceInfo.market,fallbackUsed:priceInfo.fallbackUsed});}
      ...
      if(p==='/api/dashboard'&&method==='GET'){const expiredChanged=markExpiredNowPaymentsDeposits(db);if(expiredChanged)writeDB(db);const payload=dashboard(db,u);console.log('DEPOSIT_SERVICE_RESPONSE',{userId:u.id,provider:payload.depositService?.provider,configured:Boolean(payload.depositService?.configured),missing:payload.depositService?.missing||[]});try{const priceInfo=await hb9PriceSource(db,{interval:'1d',limit:1});payload.hb9PriceSource={source:priceInfo.source,price:priceInfo.price,buyPrice:priceInfo.buyPrice,sellPrice:priceInfo.sellPrice,fallbackUsed:priceInfo.fallbackUsed};payload.settings.market={...payload.settings.market,hb9BasePrice:priceInfo.price,hb9BuyPrice:priceInfo.buyPrice,hb9SellPrice:priceInfo.sellPrice,price:priceInfo.price,buyPrice:priceInfo.buyPrice,sellPrice:priceInfo.sellPrice,source:priceInfo.source,fallbackUsed:priceInfo.fallbackUsed};payload.settings.hb9Price=priceInfo.price;}catch(error){payload.hb9PriceSource={source:'unavailable',price:null,buyPrice:null,sellPrice:null,fallbackUsed:false,error:error.message};}return send(res,200,payload);}
      ...
      if(p==='/api/deposits'&&method==='POST'){try{markExpiredNowPaymentsDeposits(db);const input=await body(req),result=await createNowPaymentsDeposit(db,u.id,input.amount);writeDB(db);return send(res,201,{message:'NOWPayments deposit created',deposit:result.deposit,payment:result.payment,service:result.service});}catch(error){return send(res,400,{error:error.message});}}
      ...
      if(p==='/api/admin/overview'&&method==='GET'){let market=marketSettings(db),priceSource=null;try{const priceInfo=await hb9PriceSource(db,{interval:'1d',limit:1});priceSource={source:priceInfo.source,price:priceInfo.price,buyPrice:priceInfo.buyPrice,sellPrice:priceInfo.sellPrice,fallbackUsed:priceInfo.fallbackUsed};market={...market,hb9BasePrice:priceInfo.price,hb9BuyPrice:priceInfo.buyPrice,hb9SellPrice:priceInfo.sellPrice,price:priceInfo.price,buyPrice:priceInfo.buyPrice,sellPrice:priceInfo.sellPrice,source:priceInfo.source,fallbackUsed:priceInfo.fallbackUsed};}catch(error){priceSource={source:'unavailable',price:null,buyPrice:null,sellPrice:null,fallbackUsed:false,error:error.message};}return send(res,200,{users:db.users.filter(x=>x.role==='user').map(x=>({...safeUser(x),summary:dashboard(db,x)})),settings:{...db.settings,hb9Price:priceSource.price,market},hb9PriceSource:priceSource,sweepService:sweepServiceStatus(),marketSettings:market,priceHistory:db.hb9_price_history||[],marketReport:hb9MarketReport(db),supply:db.hb9_supply,reserveWallets:db.reserve_wallets||[],reserveLedger:db.reserve_ledger||[],burnLedger:db.burn_ledger||[],walletLedger:db.wallet_ledger||[],exchangeOrders:db.exchange_orders||[],exchangeReserve:exchangeReserveReport(db),incomeEmissions:db.income_emissions||[],solvency:solvencyReport(db),deposits:db.deposits,depositAddresses:db.deposit_addresses||[],blockchainTransactions:db.blockchain_transactions||[],sweepTransactions:db.sweep_transactions||[],auditLogs:db.auditLogs||[],adminFundTransfers:db.admin_fund_transfers||[],conversions:db.conversions||[],stakes:db.stakes,withdrawals:db.withdrawals,transfers:db.transfers||[],ledger:db.incomeLedger,referrals:db.referralLedger||[],levelIncomeLedger:db.level_income_ledger||[],salaryRanks:db.salary_ranks||[],salaryQualifications:db.salary_qualifications||[],salaryPayouts:db.salary_payouts||[],globals:db.globalTeamRecords,flushes:db.flushRecords,directBusinessAudit:db.directBusinessAudit||[],dailyRuns:db.dailyRuns||[],schedulerRuns:db.schedulerRuns||{},demoMode:DEMO_MODE});}
      ...
      if(p.startsWith('/api/admin/withdrawals/')&&p.endsWith('/payout')&&method==='POST'){const wd=db.withdrawals.find(x=>x.id===p.split('/')[4]),input=await body(req),txHash=String(input.txHash||wd?.txHash||'').trim().toLowerCase();if(!wd||!['pending','broadcasted','failed'].includes(String(wd.status||'')))return send(res,400,{error:'Withdrawal not available for emergency payout reconciliation'});if(!/^0x[a-f0-9]{64}$/.test(txHash))return send(res,400,{error:'Transaction hash is required for emergency payout reconciliation'});const now=new Date().toISOString();Object.assign(wd,{status:'approved',txHash,fromAddress:input.fromAddress||wd.fromAddress||null,toAddress:wd.toAddress||wd.address,paidAt:now,paidBy:u.id,confirmedAt:wd.confirmedAt||now,emergencyFallback:true,updatedAt:now});walletEntry(db,{userId:wd.userId,asset:'USDT',direction:'payout',amount:wd.amount,reason:'USDT withdrawal emergency payout recorded',refId:wd.id,type:'WITHDRAWAL_EMERGENCY_PAYOUT'});audit(db,'WITHDRAWAL_EMERGENCY_PAYOUT_RECORDED',{withdrawalId:wd.id,userId:wd.userId,txHash,adminId:u.id});writeDB(db);return send(res,200,{message:'Emergency withdrawal payout recorded with transaction hash',withdrawal:wd});}
      ...
      if(p==='/api/admin/settings'&&method==='PUT'){const input=await body(req); const allowed=['dailyRoi','directMultiplier','referralPercent','globalActivityMin','globalActivityMax','minWithdrawal','maxWithdrawal','withdrawalFeePercent','manualWithdrawalApproval','treasuryWalletBSC']; for(const k of allowed)if(input[k]!==undefined)db.settings[k]=input[k];db.settings.globalPointValue=0.02;delete db.settings.globalExtraPercent; const numeric=['dailyRoi','directMultiplier','referralPercent','globalActivityMin','globalActivityMax','minWithdrawal','maxWithdrawal','withdrawalFeePercent']; if(!/^0x[a-fA-F0-9]{40}$/.test(String(db.settings.treasuryWalletBSC||'')))return send(res,400,{error:'Treasury wallet must be a valid EVM address'});if(numeric.some(k=>db.settings[k]!==undefined&&!Number.isFinite(Number(db.settings[k])))||db.settings.dailyRoi<1||db.settings.dailyRoi>4||db.settings.directMultiplier<1||db.settings.referralPercent<0||db.settings.referralPercent>100||db.settings.globalActivityMin<5||db.settings.globalActivityMax>15||db.settings.globalActivityMax<db.settings.globalActivityMin||db.settings.minWithdrawal<0||db.settings.maxWithdrawal<0||db.settings.withdrawalFeePercent<0||db.settings.withdrawalFeePercent>100)return send(res,400,{error:'Invalid settings. ROI must be 1-4%, referral percentage must be 0-100%, free Global Team must be 5-15, and withdrawal settings must be valid.'}); numeric.forEach(k=>{if(db.settings[k]!==undefined)db.settings[k]=Number(db.settings[k])});db.settings.priceMode=marketSettings(db).manualOverrideEnabled?'manual_override':'icp_proxy';writeDB(db);return send(res,200,{message:'Settings saved',settings:{...db.settings,market:marketSettings(db)}});}
      ...
      if(p==='/api/admin/salary/run'&&method==='POST')return send(res,410,{error:'Manual salary payout is disabled. Salary runs automatically on the 1st and 16th of every month.'});
